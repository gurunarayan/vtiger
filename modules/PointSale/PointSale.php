 ...
 ********************************************************************************/
 // This is the proforma vtDZiner PointSale.php script which will get converted as the written out as PointSale.php
...
// vtiger_pointsale is a custom vtiger entity generated by vtDZiner Module DZiner
class PointSale extends CRMEntity {
	var $db;

	var $module_name="PointSale";
	var $table_name = "vtiger_pointsale";
	var $table_index= 'pointsaleid';

	var $tab_name = Array('vtiger_crmentity','vtiger_pointsale','vtiger_pointsalecf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_pointsale'=>'pointsaleid','vtiger_pointsalecf'=>'pointsaleid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_pointsalecf', 'pointsaleid');
	...
	var $list_link_field= 'pointsalename';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'pointsalename');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'pointsalename';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_pointsalegrouprelation','pointsaleid');
	function PointSale() {
		$this->log = LoggerManager::getLogger('pointsale');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('PointSale');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['POINTSALE_SORT_ORDER'] != '')?($_SESSION['POINTSALE_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'PointSalename')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['POINTSALE_ORDER_BY'] != '')?($_SESSION['POINTSALE_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_pointsale.pointsaleid,  vtiger_pointsale.pointsalename, vtiger_pointsale.dateclosed FROM vtiger_pointsale, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_pointsale.pointsaleid, vtiger_pointsale.pointsalename, vtiger_crmentity.smcreatorid FROM vtiger_pointsale inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_pointsale.pointsaleid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('PointSale',$current_user);
		if($where != "")
	...
	* @param reference variable - where condition is passed when the query is executed
	* Returns Export PointSale Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("PointSale", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_pointsale
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_pointsale.pointsaleid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_pointsalecf on vtiger_pointsalecf.pointsaleid=vtiger_pointsale.pointsaleid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_pointsale.pointsaleid, vtiger_pointsale.pointsalename, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_pointsale
					inner join vtiger_contpointsalerel on vtiger_contpointsalerel.pointsaleid = vtiger_pointsale.pointsaleid
					inner join vtiger_contactdetails on vtiger_contpointsalerel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_pointsale.pointsaleid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_pointsale on vtiger_pointsale.pointsaleid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'PointSale'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_pointsale ON vtiger_pointsale.pointsaleid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_pointsale.pointsaleid = $id";
	...
	/**
	* Function to get PointSale related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('PointSale',$query,$id);
	}


	  /**
	  * Function to get PointSale related Quotes
	  * @param  integer   $id  - pointsaleid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'pointsale_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_pointsale.pointsalename from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_pointsale on vtiger_pointsale.pointsaleid=vtiger_quotes.pointsaleid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contpointsalerel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contpointsalerel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contpointsalerel"=>"pointsaleid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"pointsaleid","vtiger_salesorder"=>"pointsaleid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_pointsale","pointsaleid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityPointSale on vtiger_crmentityPointSale.crmid=vtiger_pointsale.pointsaleid and vtiger_crmentityPointSale.deleted=0
		left join vtiger_pointsalecf on vtiger_pointsalecf.pointsaleid = vtiger_pointsale.pointsaleid
		left join vtiger_groups vtiger_groupsPointSale on vtiger_groupsPointSale.groupid = vtiger_crmentityPointSale.smownerid
		left join vtiger_users as vtiger_usersPointSale on vtiger_usersPointSale.id = vtiger_crmentityPointSale.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_pointsale"=>"pointsaleid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_pointsale"=>"pointsaleid"),
			"Quotes" => array("vtiger_quotes"=>array("pointsaleid","quoteid"),"vtiger_pointsale"=>"pointsaleid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("pointsaleid","salesorderid"),"vtiger_pointsale"=>"pointsaleid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_pointsale"=>"pointsaleid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-PointSale Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contpointsalerel WHERE pointsaleid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		global $adb;
 		...
 		if($eventType == 'module.postinstall') {
			...
			$adb->pquery("INSERT into vtiger_modentity_num values(?,?,?,?,?,?)",array($adb->getUniqueId("vtiger_modentity_num"),'PointSale','POS',000001,1,1));
			
			$fieldid = $adb->getUniqueID('vtiger_field');

			$adb->pquery("INSERT INTO vtiger_field(tabid, fieldid, columnname, tablename, generatedtype, uitype, fieldname, fieldlabel, readonly, presence, maximumlength, defaultvalue, sequence, block, displaytype, typeofdata, quickcreate, quickcreatesequence, info_type, summaryfield) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", array(14, $fieldid, 'is_pos', 'vtiger_productcf', 2, 56, 'is_pos', 'Is POS?', 1, 2, 100,'', $seq, 31, 1, 'C~O', 0, 1, 'BAS', 1));			
			
			$adb->pquery("INSERT INTO vtiger_field(tabid, fieldid, columnname, tablename, generatedtype, uitype, fieldname, fieldlabel, readonly, presence, maximumlength, defaultvalue, sequence, block, displaytype, typeofdata, quickcreate, quickcreatesequence, info_type, summaryfield) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", array(14, ($fieldid+2), 'barcode', 'vtiger_productcf', 2, 1, 'barcode', 'Barcode', 1, 2,100, '', ($seq+1), 31, 1, 'V~O~LE~100', 1, 0, 'BAS', 1));			
				
			$adb->pquery("UPDATE vtiger_field_seq SET id=?",array($fieldid+2));
			
			
			$fieldid_settings = $adb->getUniqueID('vtiger_settings_field');
			$seq_res_sett = $adb->query("SELECT max(sequence) AS max_seq FROM vtiger_settings_field WHERE blockid =4");
			$seq_sett = 1;
			if ($adb->num_rows($seq_res_sett) > 0) {
				$cur_seq = $adb->query_result($seq_res_sett, 0, 'max_seq');
				if ($cur_seq != null)	$seq_sett = $cur_seq + 1;
			}

			$adb->pquery('INSERT INTO vtiger_settings_field(fieldid, blockid, name, iconpath, description, linkto, sequence,active, pinned)
				VALUES (?,?,?,?,?,?,?,?,?)', array($fieldid_settings, 4, 'POS Settings', 'portal_icon.png', 'POS Settings', 'index.php?module=PointSale&view=Settings', $seq_sett, 0, 1));
				
			$adb->pquery("UPDATE vtiger_settings_field_seq SET id=?",array($fieldid_settings));
			
			$pos_setting_count = $adb->num_rows($adb->pquery("SELECT * FROM vtiger_pos_settings", array()));
			if($pos_setting_count == 0){
				$adb->pquery("INSERT INTO vtiger_pos_settings(tax, currency) VALUES(?, ?)", array(1, '10,20,50'));
			}
			
			$custom_view_cnt = $adb->num_rows($adb->pquery("SELECT * FROM vtiger_customview WHERE entitytype=?",array('PointSale')));
			$cvid = $adb->getUniqueID('vtiger_customview');
			if($custom_view_cnt == 0){
				$adb->pquery("INSERT INTO vtiger_customview(`cvid`, `viewname`, `setdefault`, `setmetrics`, `entitytype`, `status`, `userid`) 
							VALUES(?, ?, ?, ?, ?, ?, ?)", array($cvid, 'All', 1, 0, 'PointSale', 0, 1));
				
				$adb->pquery("INSERT INTO `vtiger_cvcolumnlist`(`cvid`, `columnindex`, `columnname`) VALUES (?, ?, ?)", 
								array($cvid, 0, 'vtiger_pointsale:pointsalename:pointsalename:PointSale_PointSale_Name:V'));
				
				$adb->pquery("INSERT INTO `vtiger_cvcolumnlist`(`cvid`, `columnindex`, `columnname`) VALUES (?, ?, ?)", 
								array($cvid, 1, 'vtiger_crmentity:smownerid:assigned_user_id:PointSale_Assigned_To:V'));
			}
